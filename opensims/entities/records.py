...
_OPENSIMS_NAMESPACE = uuid.UUID("6f1d3a2e-7c4b-5a9e-8d12-0a1b2c3d4e5f")
...
def stable_id_for(
    ...
    if _is_uuid(key):
        return str(key)
    return str(uuid.uuid5(_OPENSIMS_NAMESPACE, f"{namespace}:{key}"))
...
def record_id_for(*, stable_id: str, version: int, kind: str) -> str:
    ...
    return str(uuid.uuid5(_OPENSIMS_NAMESPACE, f"{kind}:{stable_id}:{version}"))
