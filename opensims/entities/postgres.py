...
:class:`OpenSimsError` when no live pool is present.
...
class PostgresEntityRegistry:
    ...
    :class:`OpenSimsError` — the in-memory ``EntityRegistry`` remains the default.
    ...
    async def register(self, record: EntityRecord) -> EntityRecord:
        ...
        :class:`OpenSimsError` (true content-address violation), mirroring the
    ...
    async def new_version(
        ...
        *,
        ...
        unique-violation, which is surfaced as an :class:`OpenSimsError` so no
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                except Exception as exc:  # noqa: BLE001 - normalise to OpenSimsError
