...
when ``OPENSIMS_INTERNAL_API_KEY`` is set — guards every route behind a
...
from opensims.api.models import ErrorResponse
...
def _internal_key_dependency():
    ...
    When ``OPENSIMS_INTERNAL_API_KEY`` is set, every request must carry a matching
    value in the header named by ``OPENSIMS_INTERNAL_HEADER`` (constant-time
...
def create_app(container: ApiContainer | None = None) -> FastAPI:
    """Create and configure the OpenSims FastAPI app.
    ...
    handler mapping :class:`OpenSimsError` to a structured 400 (AAEO-9).
    """
    from opensims.services.observability import (
    ...
    app = FastAPI(
        title="OpenSims API",
        version="0.1.0",
        description="Backend-for-frontend over the OpenSims application services.",
    ...
    # AAEO-9: a global exception handler so OpenSimsError surfaces as a structured
    # error body instead of an opaque 500.
    @app.exception_handler(OpenSimsError)
    async def _opensims_error_handler(
        request: Request, exc: OpenSimsError
    ) -> JSONResponse:
        """Map a domain error to a structured 400 with a stable envelope."""
        return JSONResponse(
            status_code=400,
            content=ErrorResponse(detail=str(exc), code="opensims_error").model_dump(),
        )
