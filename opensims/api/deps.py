...
production, set ``OPENSIMS_DATABASE_URL`` and use
...
class ApiContainer:
    ...
    def build_default(cls) -> ApiContainer:
        ...
        ``OPENSIMS_DATABASE_URL`` is set. For a Postgres-backed store use
    ...
    async def build_default_async(cls) -> ApiContainer:
        """Async container builder that honours ``OPENSIMS_DATABASE_URL`` (AAEO-2).

        When ``OPENSIMS_DATABASE_URL`` is set, constructs a
    ...
    def _build_inference() -> InferenceService:
        ...
        from opensims.services.inference.models import GatewayRuntimeConfig
        from opensims.services.inference.service import InferenceService

        if os.environ.get("PYDANTIC_AI_GATEWAY_API_KEY"):
            region = os.environ.get("OPENSIMS_GATEWAY_REGION", "us")
