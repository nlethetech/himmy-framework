...
persona + task into an answered :class:`~opensims.agents.base_agent.thread.ChatThread`
...
if TYPE_CHECKING:  # pragma: no cover - typing only, avoids import cycles
    ...
    from opensims.services.storage.service import MemoryStore
...
class SingleAgentRuntime:
    ...
    async def _run_task_body(
        ...
        *,
        ...
        from opensims.agents.base_agent.thread import Message, MessageRole
    ...
    async def _resolve_snapshot(
        ...
        re-raised as an :class:`OpenSimsError` so the caller knows the requested
        ...
        if self.context_service is None:
            if requested and self.strict_snapshot:
                raise OpenSimsError(
                    "snapshot requested but no context_service is wired"
                )
            return (
                None,
                resolved_id,
        ...
        if snapshot is not None:
            ...
            if requested and self.strict_snapshot:
                raise OpenSimsError(snapshot_error)
...
def _timeout(seconds: float):
    ...
    if timeout_cm is None:  # pragma: no cover - 3.10 fallback only
        raise OpenSimsError("deadline_seconds requires Python 3.11+ (asyncio.timeout)")
