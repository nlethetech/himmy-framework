"""Runtime kernel: a one-call builder for the full offline-first OpenSims stack.
...
the common case is::
    ...
    from opensims import build_runtime
...
:class:`~opensims.services.inference.client_manager.StubClientManager` (no network,
no keys). A real pydantic-ai-backed manager is selected ONLY when ``pydantic_ai``
is importable AND a provider key is present AND ``OPENSIMS_EXAMPLES_MODEL`` names a
