...
from opensims.services.mcp.models import MCPServerSpec, MCPTool, MCPToolResult
...
class MCPClient:
    ...
    async def aclose(self) -> None:
        ...
        if self._closed:
            return
        self._closed = True
        self._fail_pending(OpenSimsError("MCP client closed"))
    ...
    async def request(self, method: str, params: dict[str, Any] | None = None) -> Any:
        ...
        if self._closed:
            raise OpenSimsError("MCP client is closed.")
        self._next_id += 1
    ...
    async def _write(self, message: dict[str, Any]) -> None:
        ...
        if self._proc.stdin is None:  # pragma: no cover - defensive
            raise OpenSimsError("MCP server stdin is not available.")
    ...
    async def _read_loop(self) -> None:
        ...
        while True:
            ...
            if not line:  # EOF — the server exited or closed its stream
                self._fail_pending(OpenSimsError("MCP server closed the connection"))
                return
            stripped = line.strip()
            if not stripped:
                continue
