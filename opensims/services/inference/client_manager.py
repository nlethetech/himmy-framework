...
class StubClientManager:
    ...
    async def _run_workflow(
        ...
        if state is None:
            raise OpenSimsError("WORKFLOW requires request.workflow to be set")
...
class GatewayClientManager:
    ...
    ``OPENSIMS_GATEWAY_STUB_FALLBACK`` is set) or raises a clear, actionable error.
    ...
    async def generate(self, request: InferenceRequest) -> InferenceResponse:
        ...
        if os.environ.get("OPENSIMS_GATEWAY_STUB_FALLBACK"):
        ...
        raise OpenSimsError(
            ...
            "PYDANTIC_AI_GATEWAY_API_KEY (set OPENSIMS_GATEWAY_STUB_FALLBACK=1 "
            "to simulate offline)."
        )
