...
:class:`~opensims.services.storage.service.StorageService` surface 1:1 and are a
...
- The ``ai_call_log`` view flattens the request/response event pair into one row
  per LLM call. See the payload-key contract in :mod:`opensims.core.events`.
"""
...
-- populate the column. See opensims.core.events for the payload-key contract.
...
def _require_asyncpg() -> Any:
    ...
    try:
        ...
        raise OpenSimsError(
            ...
            "(pip install 'opensims[postgres]') and a running database."
        ) from exc
    return asyncpg
...
class PostgresStorageService:
    ...
    def knowledge_backend(self) -> Any:
        ...
        """
        from opensims.services.knowledge.backend import PgVectorKnowledgeBackend
    ...
    async def create_knowledge_schema(
        ...
        *,
        ...
        :class:`~opensims.services.knowledge.backend.PgVectorKnowledgeBackend`.
        """
    ...
    def _require_pool(self) -> Any:
        """Return the live pool or raise a clear OpenSimsError."""
