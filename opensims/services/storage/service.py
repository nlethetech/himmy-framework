"""Storage kernel: the in-memory async persistence service + MemoryStore protocol.

``StorageService`` is the default backend: process-local dicts, an async API, and
enough surface to satisfy both :class:`~opensims.core.events.EventSink` and the
:class:`MemoryStore` protocol. Cross-kernel types (ContextField, ContextSnapshot,
EvaluationRun) are only referenced under ``TYPE_CHECKING`` to avoid import cycles;
at runtime the stored objects are kept as opaque values keyed by id.
"""
...
from opensims.core.events import RunEvent
...
if TYPE_CHECKING:  # pragma: no cover - typing only, avoids storage <-> context cycle
    ...
    from opensims.services.evaluation.models import EvaluationRun


@runtime_checkable
class MemoryStore(Protocol):
    """The conversation + event subset of the storage surface used by the runtime."""

    async def save_thread(self, thread: Any) -> Any:
        """Persist (upsert) a chat thread by its id."""
        ...
    ...
    async def append_event(self, event: RunEvent) -> None:
        """Append a run event to the audit stream."""
        ...

    async def list_events(
        self, thread_id: str | None = None, trace_id: str | None = None
    ) -> list[RunEvent]:
        """List events, optionally filtered by thread or trace id."""
        ...
...
class StorageService:
    ...
    Satisfies :class:`~opensims.core.events.EventSink` (``append_event``) and the
    :class:`MemoryStore` protocol. All state is process-local and lost on exit.
    """
    ...
    def __init__(self) -> None:
        ...
        self._context_evidence: list[Any] = []
    ...
    async def save_context_evidence(self, record: Any) -> Any:
        ...
        self._context_evidence.append(record)
        return record
...
__all__ = ["MemoryStore", "StorageService"]
