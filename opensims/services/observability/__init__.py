...
When ``OPENSIMS_LOGFIRE_ENABLED`` is unset or falsy this entire module is a
...
attributes unless ``OPENSIMS_LOGFIRE_INCLUDE_CONTENT`` is truthy.
"""
...
def configure_observability() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "OPENSIMS_LOGFIRE_ENABLED is set but the 'logfire' package is not "
            "installed; install the observability extra: "
            "pip install 'opensims[observability]'"
        ) from exc

    service_name = os.environ.get("OPENSIMS_LOGFIRE_SERVICE_NAME", "opensims")
    ...
    os.environ.setdefault(
        "OPENSIMS_LOGFIRE_INCLUDE_CONTENT", "true" if include_content else "false"
    )
    _CONFIGURED = True
...
def _span_attributes(event: Any) -> dict[str, Any]:
    ...
    for label, value in (
        ("opensims.trace_id", getattr(event, "trace_id", None)),
        ("opensims.thread_id", getattr(event, "thread_id", None)),
        ("opensims.agent_id", getattr(event, "agent_id", None)),
        ("opensims.request_id", getattr(event, "request_id", None)),
        ("opensims.tool_call_id", getattr(event, "tool_call_id", None)),
        ("opensims.latency_ms", getattr(event, "latency_ms", None)),
        ("opensims.cost", getattr(event, "cost", None)),
        ("opensims.error", getattr(event, "error", None)),
    ):
    ...
    for key, value in payload.items():
        ...
        if not include and key in _CONTENT_KEYS:
            continue
        attributes[f"opensims.payload.{key}"] = value
    return attributes
...
def emit_event_span(event: Any) -> None:
    ...
    ``OPENSIMS_LOGFIRE_INCLUDE_CONTENT`` is on. Never raises.
    """
...
def instrument_fastapi(app: Any) -> None:
    ...
    Soft-degrades with a one-line warning when ``OPENSIMS_LOGFIRE_ENABLED`` is on
    ...
    try:  # pragma: no cover - requires logfire[fastapi]
        ...
        print(
            "[opensims.observability] FastAPI instrumentation unavailable "
...
def instrument_asyncpg() -> None:
    ...
    try:  # pragma: no cover - requires logfire[asyncpg]
        ...
        print(
            "[opensims.observability] asyncpg instrumentation unavailable "
