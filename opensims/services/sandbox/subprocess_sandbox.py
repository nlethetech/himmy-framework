...
UNTRUSTED code, run OpenSims inside an OS-level isolate (container / gVisor /
seccomp / microVM) or supply a :class:`~opensims.services.sandbox.base.Sandbox`
...
class SubprocessSandbox:
    ...
    async def run_code(
        ...
        *,
        ...
        limits = self._limits
        with tempfile.TemporaryDirectory(prefix="opensims-sbx-") as workdir:
            work = Path(workdir)
    ...
    async def _run(
        ...
        try:
            ...
            raise OpenSimsError(f"sandbox failed to start a subprocess: {exc}") from exc
