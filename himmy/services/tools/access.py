...
_READ_VERBS = frozenset(
    {
        "get", "list", "read", "search", "fetch", "find", "show", "view", "status",
        "summary", "summarize", "count", "recall", "query", "totals", "total",
        "recent", "history", "overview", "describe", "inspect", "latest", "info",
        "lookup", "check", "report", "stats",
    }
)
# Verb tokens that strongly imply a write / action (changes state or acts on the world).
_WRITE_VERBS = frozenset(
    {
        "write", "log", "add", "record", "send", "create", "update", "delete",
        "remove", "set", "ingest", "remember", "complete", "save", "post", "put",
        "submit", "schedule", "cancel", "assign", "pay", "transfer", "execute",
        "run", "make", "register", "book", "order", "approve", "reject", "upsert",
    }
)
