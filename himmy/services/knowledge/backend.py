...
skip when ``HIMMY_TEST_POSTGRES_DSN`` is unset.
"""
...
def resolve_column_and_index(
    ...
    *,
    ...
    """
    if vector_dim <= 0:
        raise HimmyError("vector_dim must be a positive integer.")
    ...
    if idx not in ("ivfflat", "hnsw"):
        raise HimmyError(f"index_method must be 'ivfflat' or 'hnsw', got {idx!r}.")
    return col, idx
...
class PgVectorKnowledgeBackend:
    ...
    :class:`~himmy.services.knowledge.service.KnowledgeBase` via
    ...
    def __init__(self, pool: Any) -> None:
        ...
        if pool is None:
            raise HimmyError("PgVectorKnowledgeBackend requires a live asyncpg pool.")
        self._pool = pool
