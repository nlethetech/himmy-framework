...
def _load_validated_yaml(path: Path) -> dict[str, dict[str, str]]:
    ...
    Raises :class:`HimmyError` with an actionable message on malformed input.
    """
    resolved = Path(path).resolve()
    if not resolved.exists():
        raise HimmyError(f"prompt template not found: {resolved}")
...
class PromptTemplate:
    ...
    (path, mtime); malformed structure raises a clear :class:`HimmyError`.
    """
