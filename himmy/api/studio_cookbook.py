...
class CookbookStore:
    ...
    def upsert(self, r: Recipe, *, workspace_id: str | None = None) -> Recipe:
        ...
        Re-stamp / content-tamper guard: a bound tenant upserting onto an id it is NOT
        entitled to mutate (another tenant's row or a legacy NULL row) ABORTS the upsert and
        returns the existing recipe UNCHANGED — the ``INSERT OR REPLACE`` keyed only on id
        would otherwise let a bound tenant re-stamp/steal AND clobber the *content*
        (name/agent_path/prompt/notes) of a shared/legacy recipe that must be read-visible but
        IMMUTABLE. ``workspace_id is None`` (offline) is unchanged.
        """
        stamp = workspace_id
        if workspace_id is not None:
            ...
            existing = self._conn.execute(
                "SELECT * FROM recipes WHERE id = ?", (r.id,)
            ).fetchone()
            if existing is not None and not row_writable_in_scope(
                existing["workspace_id"], workspace_id
            ):
                # A bound tenant may not mutate a legacy/foreign-owned row at all: abort.
